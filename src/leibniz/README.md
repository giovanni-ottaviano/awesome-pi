...
However, there are lots of *acceleration techniques* that can be implemented to increase the rate of convergence.
For example, **Shanks transformation** $S$
...
It's possible to get even more correct digits of $\pi$ just by tuning the input parameters, but requesting too many transformations or orders can result in precision loss due to floating-point arithmetic and finite machine precision. 